...
class ParserSimulator(simulation.Simulator):
    ...
    # def create_stan_data_scenario(self, n_users=10, pct_items=0.5, uerr_a=3, uerr_b=5, difficulty_a=5, difficulty_b=20,
    #                                 n_gold_users=0, gold_user_err=0.01):
    #     # err_rates = np.random.beta(uerr_a, uerr_b, n_users)
    #     # err_rates[:n_gold_users] = gold_user_err
    #     # difficulty_dict, _ = simulation.create_item_param_dicts(self.df.sentenceId, difficulty_a, difficulty_b, 0, 0)
